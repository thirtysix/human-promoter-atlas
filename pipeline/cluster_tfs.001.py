...
SIGNAL_FLOOR    = 0.02       # drop TFs with row.max < 2% of TSSs bound
SMOOTH_SIGMA    = 10         # Gaussian smoothing sigma in bp
K_CLUSTERS      = 8          # number of clusters to cut the tree at
...
def main():
    ...
    plot_clustermap(M_norm, labels, Z,
                    out_path_stem=str(plots_dn / "tf_clustermap.binary"))

    print("plotting per-cluster profiles...")
    plot_cluster_profiles(M, M_norm, labels,
                          out_path_stem=str(plots_dn / "tf_cluster_profiles"))

    print("plotting UMAP...")
    plot_umap(M_norm, labels,
              out_path_stem=str(plots_dn / "tf_umap"),
              random_state=RANDOM_STATE)
    ...
    plot_cluster_histograms(hist_matrix, labels,
                            out_dn=plots_dn / "cluster_histograms",
